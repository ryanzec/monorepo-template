...
- `packages/utils/*` - Utility methods, each file should be grouped by logic functionality (`string.ts` for `stringUtils`, `user.ts` for `userUtils`, etc.) 
...
If tested need but will do later (time crunch or hgihly experimental feature), do this
...
describe('api utils', () => {
  it('test when ready', () => {
      // @todo(testing) add tests when code is ready for them
...
If not needed as the file as not testable code for whatever reason like this
