...
import { Container, Icon, IconSvg } from '$components/button/button.css';
  ...
  return (
    ...
    >
      {isLoading && loadingIconPosition === ButtonIconPosition.PRE && (
        <span className={Icon} data-id="pre-loading-icon" data-position={ButtonIconPosition.PRE} data-state={dataState}>
          <FontAwesomeIcon className={IconSvg} icon={faSpinner} />
        </span>
      )}
      {!isLoading && preIcon && (
        <span className={Icon} data-id="pre-icon" data-position={ButtonIconPosition.PRE} data-state={dataState}>
          {preIcon}
        </span>
      )}
      {children}
      {!isLoading && postIcon && (
        <span className={Icon} data-id="post-icon" data-position={ButtonIconPosition.POST} data-state={dataState}>
          {postIcon}
        </span>
      )}
